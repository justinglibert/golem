...
def learner(
    ...
    world, impala_group, server = init
    ...
    buff = glm.buffers.CentralizedBuffer(200, impala_group)
    ...
    with prof(category="sleeping"):
        while buff.size() < B:
            logger.info(
                f"The size of the replay buffer ({buff.size()}) is smaller then {B}. Waiting..."
            )
            sleep(5)

    for iteration in it.count():
        with prof(category="rpc-sample"):
            bsize, buffers = buff.sample(B, glm.buffers.DistributedBuffer.sample_method_random_unique)
            if bsize == 0:
                logger.warning("Batch size is 0!")
                sleep(1)
                continue
        with prof(category="preprocessing-0"):
            keys = buffers[0].keys()
        ...
        with prof(category="backward_pass"):
            ...
            mean_ep_returns_moving, var_ep_returns_moving = moving_episode_returns.stats() 
            stats = {
...
def actor(
    ...
    for iteration in it.count():
        ...
        if iteration % 100 == 0:
            with prof(category="recreating-buffers"):
                del buffers
                buffers = _create_buffers(
        ...
        with prof(category="replay-buffer"):
            impala_group.registered_async("add_to_buffer", args=(buffers,))
...
def init(cfg: DictConfig):
    ...
    return (world, impala_group, server)
